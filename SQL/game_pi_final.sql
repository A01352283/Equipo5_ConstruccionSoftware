...
drop trigger update_rhythm_best_score;
...
FOR EACH ROW
BEGIN
    IF OLD.trivia_best_score < NEW.trivia_last_score
    THEN
        SET trivia_best_score = trivia_last_score;
    END IF ;
END$$
DELIMITER ;

drop trigger update_memory_best_score;
...
FOR EACH ROW
BEGIN
    IF OLD.memory_best_score < NEW.memory_last_score
    THEN
        SET memory_best_score = memory_last_score;
    END IF ;
END$$
DELIMITER ;

drop trigger update_memorysounds_best_score;
...
FOR EACH ROW
BEGIN
    IF OLD.memorysounds_best_score < NEW.memorysounds_last_score
    THEN
        SET memorysounds_best_score = memorysounds_last_score;
    END IF ;
END$$
DELIMITER ;
